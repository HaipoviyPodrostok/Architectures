...
В отличие от предыдущего пункта, здесь используется 2-битный предсказатель с начальным значением TAKE.

Механизм работы в одной ячейке:

Начало: Ячейка находится в состоянии сильной уверенности 0/T (TAKE).

Исполняется magic_br_1: Его фактический результат всегда Not Taken (NT). Предсказатель ошибается (Incorrect +1), но из-за 2-битной логики он не меняет прогноз на NOT TAKE сразу. Он лишь переходит в состояние «слабой уверенности» 1/T (TAKE).

Исполняется magic_br_2: Его фактический результат всегда Taken (T). Предсказатель видит прогноз TAKE, и он оказывается верным (Correct +1). Автомат возвращается в состояние сильной уверенности 0/T (TAKE).

Цикл повторяется: На каждой итерации magic_br_1 всегда ошибается, а magic_br_2 всегда угадывает.

Итог: Благодаря «инерции» 2-битного автомата, второе ветвление (magic_br_2) постоянно «вытягивает» предсказание обратно в TAKE, не давая первой инструкции переключить ячейку в режим NOT TAKE. В результате ровно половина предсказаний оказывается верной.
